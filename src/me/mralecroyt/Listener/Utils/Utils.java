...
public class Utils
{
    ...
    public static boolean connectToBungeeServer(final Player player, final String server) {
        try {
            ...
            if (!messenger.isOutgoingChannelRegistered(CoreMain.getInstance(), "BungeeCord")) {
                messenger.registerOutgoingPluginChannel(CoreMain.getInstance(), "BungeeCord");
            }
            if (server.length() == 0) {
            ...
            out.writeUTF(server);
            player.sendPluginMessage(CoreMain.getInstance(), "BungeeCord", byteArray.toByteArray());
        }
        catch (Exception ex) {
            ex.printStackTrace();
